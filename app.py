...
if st.button("guide"):
    st.subheader('How to use the webapp')
    st.image("gd.jpg")
    st.markdown("""
    <div style="rackground-color: #bfe6f2; padding: 10px; border-radius: 5px;">
...
#using dct
    ...
    m_dot = st.sidebar.number_input('mass flow rate of manifold working fluid ($\dot{m}$)',format="%.5f",min_value=0.001,max_value=1.0)
    ...
    V_amb = st.sidebar.number_input('wind velocity($V_{amb}$)',format="%.4f",min_value=0.0,max_value=12.0)
    ...
    D_o = st.sidebar.number_input('outer diameter of the tube ($D_{o}$)',format="%.4f",min_value=47.0,max_value=102.0)
    L_tube = st.sidebar.number_input('length of the tube ($L_{tube}$)',format="%.4f",min_value=1000.0,max_value=2100.0)
    L_c = st.sidebar.number_input('length of the condenser ($L_{c}$)',format="%.4f",min_value=70.0,max_value=400.0)
    L_e = st.sidebar.number_input('length of the evaporator ($L_{e}$)',format="%.4f",min_value=750.0,max_value=2000.0)
    L_a = st.sidebar.number_input('length of the adiabatic section ($L_{a}$)',format="%.4f",min_value=0.0,max_value=300.0)
    De_o = st.sidebar.number_input('outer diameter of the evaporator ($D_{e,o}$)',format="%.4f",min_value=8.0,max_value=36.0)
    t_e = st.sidebar.number_input('thickness of the evaporator wall ($t_{e}$)',format="%.4f",min_value=0.1,max_value=1.0)
    Dc_o = st.sidebar.number_input('outer diameter of the condenser ($D_{c,o}$)',format="%.4f",min_value=12.0,max_value=36.0)
    t_c = st.sidebar.number_input('thickness of the condenser wall ($t_{c}$)',format="%.4f",min_value=0.1,max_value=1.0)
    theta = st.sidebar.number_input('radiation angle ($θ$)',format="%.4f",min_value=0.0,max_value=60.0)
    t_g = st.sidebar.number_input('thickness of the glass ($t_{g}$)',format="%.4f",min_value=1.5,max_value=6.0)
    D_H = st.sidebar.number_input('hydraulic diameter of the manifold tube ($D_{H}$)',format="%.4f",min_value=20.0,max_value=270.0)
    A_man = st.sidebar.number_input('manifold area ($A_{man}$)',format="%.4f",min_value=0.002,max_value=10.0)
...
# Load and display image
    ...
    if manifold_fluid == 'Water':
        if predicted_parameter=='Temperature':
            ...
            st.sidebar.markdown("<h1 style='color: orange;'>Inlet Manifold Working Fluid Parameters </h1>", unsafe_allow_html=True)
            m_dot = st.sidebar.number_input('mass flow rate of manifold working fluid ($\dot{m}$)',format="%.5f",min_value=0.01,max_value=1.0)      
            T_in = st.sidebar.number_input('inlet temperature ($T_{in}$)',format="%.4f",min_value=10.0,max_value=90.0)
            st.sidebar.markdown("<h1 style='color: green;'>Environmental Parameters</h1>", unsafe_allow_html=True)
            T_amb = st.sidebar.number_input('ambient temperature ($T_{amb}$)',format="%.4f",min_value=10.0,max_value=50.0)
            st.sidebar.markdown("<h1 style='color: red;'>Irradiance Parameters</h1>", unsafe_allow_html=True)
...
#..................
   ...
   #.........................
    ...
    # Dictionary to map fluid names to saturation temperatures
            ...
            theta = st.sidebar.number_input('radiation angle ($θ$)',format="%.4f",min_value=0.0,max_value=60.0)
            D_o = st.sidebar.number_input('outer diameter of the tube ($D_{o}$)',format="%.4f",min_value=47.0,max_value=102.0)
            L_c = st.sidebar.number_input('length of the the condenser ($L_{c}$)',format="%.4f",min_value=70.0,max_value=400.0)
...
#..................
    ...
    # Display prediction results
        ...
        if predicted_parameter=='Temperature': 
            ...
            fluid_name = st.sidebar.radio('Heatpipe Fluid', list(fluid_alpha.keys()))
            alpha = fluid_alpha[fluid_name] # Get saturation temperature from dictionary
            st.sidebar.markdown("<h1 style='color: orange;'>Inlet Manifold Working Fluid Parameters </h1>", unsafe_allow_html=True)
            m_dot = st.sidebar.number_input('mass flow rate of manifold working fluid ($\dot{m}$)',format="%.5f",min_value=0.001,max_value=0.05)
            T_in = st.sidebar.number_input('inlet temperature ($T_{in}$)',format="%.4f",min_value=10.0,max_value=90.0)
            st.sidebar.markdown("<h1 style='color: red;'>Irradiance Parameters</h1>", unsafe_allow_html=True)
            I = st.sidebar.number_input('solar irradiance ($I$)',format="%.4f",min_value=100.0,max_value=1200.0)
            tau_g = st.sidebar.number_input('glass transmissivity ($τ_{g}$)',format="%.4f",min_value=0.8,max_value=0.94)
